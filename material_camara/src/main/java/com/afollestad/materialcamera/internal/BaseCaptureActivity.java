...
import static com.afollestad.materialcamera.internal.BaseCameraFragment.GALLERY_IMAGE_REQUEST_CODE;
...
public abstract class BaseCaptureActivity extends AppCompatActivity implements BaseCaptureInterface {
    ...
    public void onShowGallery() {
        ...
        startActivityForResult(intent, BaseCameraFragment.GALLERY_IMAGE_REQUEST_CODE);
    }
    ...
    public final void onShowPreview(@Nullable final String outputUri, boolean countdownIsAtZero) {
        if ((shouldAutoSubmit() && (countdownIsAtZero || !allowRetry() || !hasLengthLimit())) || outputUri == null) {
            ...
            useMedia(outputUri, getVideoSize(outputUri));
        } else {
            ...
            Fragment frag = PlaybackVideoFragment.newInstance(outputUri, allowRetry(),
                    getIntent().getIntExtra(CameraIntentKey.PRIMARY_COLOR, 0), getVideoSize(outputUri));
            getFragmentManager().beginTransaction()
    ...
    @Override
    public void onShowStillshot(String outputUri, long sizeInBytes) {
        if (shouldAutoSubmit()) {
            useMedia(outputUri, sizeInBytes);
        } else {
            Fragment frag = StillshotPreviewFragment.newInstance(outputUri, allowRetry(),
                    getIntent().getIntExtra(CameraIntentKey.PRIMARY_COLOR, 0), sizeInBytes);
            getFragmentManager().beginTransaction()
                    .replace(R.id.container, frag)
                    .commit();
        }
        ...
        if (requestCode == PERMISSION_RC) {
            ...
            switch (requestCode) {
                case GALLERY_IMAGE_REQUEST_CODE: {
                    if (data != null) {
                        String path = ImageUtil.getPath(this, data.getData());
                        long size = 0;
                        if (path != null) {
                            size = new File(path).length();
                        }
                        onShowStillshot(path, size);
                    } else {
                        new MaterialDialog.Builder(this)
                                .title(R.string.mcam_error)
                                .content(R.string.mcam_gallery_preview_error_message)
                                .positiveText(android.R.string.ok)
                                .dismissListener(new DialogInterface.OnDismissListener() {
                                    @Override
                                    public void onDismiss(DialogInterface dialog) {
    ...
    @Override
    public final void useMedia(String uri, long sizeInBytes) {
        if (uri != null) {
            setResult(Activity.RESULT_OK, getIntent()
                    .putExtra(MaterialCamera.STATUS_EXTRA, MaterialCamera.STATUS_RECORDED)
                    .putExtra(MaterialCamera.SIZE_EXTRA, sizeInBytes)
    ...
    public int qualityProfile() {
        return getIntent().getIntExtra(CameraIntentKey.QUALITY_PROFILE, CamcorderProfile.QUALITY_480P);
    }
    ...
    public int iconGallery() {
        return getIntent().getIntExtra(CameraIntentKey.ICON_GALLERY, R.drawable.mcam_action_gallery);
    }
    ...
    @Override
    public boolean saveOnCapture() {
        return getIntent().getBooleanExtra(CameraIntentKey.SAVE_ON_CAPTURE, false);
    }

    @Override
    public boolean saveOnStillshot() {
        return getIntent().getBooleanExtra(CameraIntentKey.SAVE_ON_STILLSHOT, false);
    }

    @Override
    public boolean compressData() {
        return getIntent().getBooleanExtra(CameraIntentKey.COMPRESS_DATA, true);
    }

    @Override
    public long getVideoSize(String outputUri) {
        if (TextUtils.isEmpty(outputUri)) return 0;
        String path = ImageUtil.getPath(this, Uri.parse(outputUri));
